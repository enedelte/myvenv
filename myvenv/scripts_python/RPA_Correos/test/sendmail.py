...
def enviar_correo_desde_alias(destinatario, asunto, cuerpo, adjunto, alias):
    ...
    if os.path.exists(ruta_completa):
        ...
        messagebox.showerror("Error", f"No se pudo encontrar el archivo: {ruta_completa}")
        return

    try:
        mail.Send()
    except Exception as e:
        messagebox.showerror("Error", f"Error al enviar correo a {destinatario}: {e}")
...
def seleccionar_archivo_csv():
    ...
    if archivo_csv:
        ...
        messagebox.showinfo("Completado", "Se han enviado por completo los correos relacionados")
...
def main():
    folder_path = filedialog.askdirectory(title="Seleccionar carpeta de documentos DOCX")
    if not folder_path:
